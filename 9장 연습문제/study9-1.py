class Dog:
    def __init__(self, name, age, trick): #인스턴스 변수를 괄호에 전부 추가하기
        self.name = name
        self.age = age
    ...
    def show_tricks(self):
        #self.trick.append(self.trick) #모든 self가 들어가는 변수는 self를 잊지 말자. 이것도 틀리긴 했지만
        print(f"{self.name}이의 장기는 {self.trick}입니다.")
'''    
def bark(name):
